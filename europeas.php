...
$xml=simplexml_load_file('http://resultados.elpais.com/elecciones/'.$year.'/'.$tipo.'/index.xml2') or die("Error: Cannot create object");
foreach ($xml->resultados->partido as $p){
    ...
    if ($p->electos > 0){
        if (intval($p->electos) > 5) {
            $grafica[] = "{name: '".str_replace("'", "", $p->nombre)."',y: $p->electos,color: '$color'}";
        }else{
            $grafica[] = "{name: '".str_replace("'", "", $p->nombre)."', y: $p->electos, color: '$color',dataLabels: {enabled: false}}";
        }
        $partidos[] = $a;
    }  else{
        $partidos2[] = $a;
  ...
  <body>      
    ...
    <div class="row">
      <div class="medium-12 columns">
          ...
          <h2>Resultados del <?echo $year;?> para Europa en <?echo $xml->nombre_sitio;?></h2>
